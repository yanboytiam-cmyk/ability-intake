/* ══════════════════════════════════════════════════════════════════════════
   ...
   Correspondance avec le PDF fourni par le cabinet :
     ...
     Acknowledge Receipt of Persons Served Handbook ... page 15
       (les deux dernieres pages sont inversees par rapport au papier,
        demande de Yanis le 16/09/2026)
...
(function () {
  ...
  function contenuOmhcConsent(p, d) {
    ...
    p.rang([['Client Signature', { sig: d.signature_image }], ['Date', d.signature_date]],
           [0.66, 0.34]);
    ligneTuteur(p, d);
    /* Le temoin reste imprime meme non signe : le papier de l'OMHC porte sa
       ligne, et le controle de conformite l'exige. Ce n'est pas une ligne de
       tuteur laissee vide, c'est une place prevue pour la seance. */
  ...
  function contenuUrgencePcp(p, d) {
    ...
    p.encadre(function () {
      ...
      /* Le papier porte une seconde ligne, prevue pour la re-signature annuelle.
         Remplie par un tuteur, elle se justifie ; vide, elle n'est qu'un trait
         de plus sous un document deja signe. */
      if (aTuteur(d)) {
        p.filet();
        p.rang([['Client/Legal Guardian Signature', { sig: d.guardian_signature_image }],
                ['Printed Name', d.guardian_printed_name],
                ['Date', d.signature_date]], [0.5, 0.32, 0.18]);
      }
    });
  }
  ...
  function contenuTelehealth(p, d) {
    ...
    p.rang([['Person\'s Signature', { sig: d.signature_image }], ['Date', d.signature_date]],
           [0.66, 0.34]);
    if (aTuteur(d)) {
      p.rang([['Parent or Guardian Signature', { sig: d.guardian_signature_image,
                nom: d.guardian_printed_name }],
              ['Date', d.signature_date]], [0.66, 0.34]);
    }
  }
  ...
  function contenuEchange(p, d) {
    ...
    p.encadre(function () {
      p.paragraphe('I agree that the information above has not changed since the last date it '
        + 'was signed.', { apres: 1 });
      /* Le papier aligne trois lignes, pour les renouvellements annuels. Le
         dossier numerique est resigne en entier a chaque fois : on n'imprime
         que les signataires reels. */
      var lignesSig = [[d.signature_image, nomImprime(d), d.signature_date]];
      if (aTuteur(d)) {
        lignesSig.push([d.guardian_signature_image, d.guardian_printed_name, d.signature_date]);
      }
  ...
  function contenuManuel(p, d) {
    ...
    if (aTuteur(d)) {
      rangs.push([{ sig: d.guardian_signature_image, libelle: 'Parent/Guardian',
                    nom: d.guardian_printed_name }, ['Date', d.signature_date]]);
    }
    rangs.push([{ sig: d.staff_signature_image, libelle: 'Program Representative',
                  nom: d.staff_print_name },
                ['Date', staff ? (nonVide(d.staff_date_signed) || d.signature_date) : '']]);
    p.tableau(rangs, [0.72, 0.28]);
  }
